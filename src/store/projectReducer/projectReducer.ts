            ...
            return {
              ...
              tasks: project.tasks.map((task: TaskType) => {
                if (task.number === action.payload.taskId) {
                  ...
                  const elapsedTime = task.startTime ? endTime - task.startTime : 0;
                  return {
                    ...task,
                    timeAtWork: (task.timeAtWork || 0) + elapsedTime,
                    startTime: null,
                    isRunning: false
                  };
                }
            ...
            return {
              ...
              tasks: project.tasks?.map((task: TaskType) => ({
                ...
                isFilteredOut:
                  ...
                  !(task.header.toLowerCase().includes(filterText) || task.number.toString().includes(filterText))
              }))
            };
          }
          return project;
    ...
    case 'FILTER_PROJECT':
      newState = {
        ...
        projects: state.projects.map((project: ProjectType) => {
          return {
            ...project,
            isFilteredOut: action.payload && !project.name.toLowerCase().includes(action.payload.toLowerCase())
          };
        })
      };
      return newState;
