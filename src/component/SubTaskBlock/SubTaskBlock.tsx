...
const SubTaskBlock = ({ taskId, projectId }: ISubTaskProps) => {
    ...
    return (
        <div className={styles.task__subtasks}>
            <h2>Подзадачи</h2>
            <div>
                {subTask.filter((subtask: SubTask) => subtask !== undefined).map((subtask: SubTask) => {
                    return (
                        <Accordion key={subtask.number} subtask={subtask} subTaskId={subtask.number} list={[{ header: subtask.header, description: subtask.description }]} />
                    )
                })}
            </div>
        </div>
