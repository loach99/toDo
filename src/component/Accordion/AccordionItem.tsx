...
export const AccordionItem = ({ onClick, isOpen, subTaskId, subtask }: IAccordionItem) => {
  ...
  const status = useSelector((state: RootState) => state.projectReducer.projects)
    ...
    .filter((task: TaskType) => task.number === number)[0]
    .subtasks.filter((subtask: SubTask) => subtask.number === subTaskId)[0].status;
  ...
  return (
    <li className={styles.accordion__item}>
      <button className={styles.accordion__header} onClick={() => onClick()}>
        {subtask.header}
        <div>{subtask.date}</div>
      </button>
      <div
        className={styles.accordion__collapse}
        style={isOpen && itemRef.current ? { height: itemRef.current.scrollHeight } : { height: '0px' }}>
        <div className={styles.accordion__body} ref={itemRef}>
          ...
          <div>Выполнить до: {subtask.endDate}</div>
        </div>
      </div>
    </li>
  );
