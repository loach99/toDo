...
import { moveTask } from '../../store/projectReducer/actions/actions';
...
const Task = () => {
  ...
  const { number, header, description, date, priority, endDate } = useSelector(
    (state: RootState) => state.taskReducer.taskWindow
  );
    ...
    <div className={styles.task}>
      <div className={styles.task__header}>
        <div className={styles.task__priority__widget}>
          <div className={changePriority(priority)}>{priority}</div>
        ...
        <div>
          <div className={styles.task__header__info}>
            <div>
              <div className={styles.task__header__name}>{header}</div>
              <div className={styles.task__header__created}>{date}</div>
              <div>№{number}</div>
            </div>
          </div>
        </div>
      ...
      <div className={styles.task__comments}>
        <div>
          ...
          <CommentBlock
            ...
            inputId={inputId}
            taskNumber={number}
            files={tasks.flat()[0].files}
            comments={tasks.flat()[0].comments}
          />
        </div>
      </div>
    </div>
