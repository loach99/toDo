...
const CreateSubTaskModal = ({ isOpen, projectId, taskId }: CreateSubtaskProps) => {
  ...
  const handleSubmit = () => {
    ...
    dispatch(addSubTask(taskId, newTask, Number(projectId)));
      ...
      <div className={styles.modal__wrap}>
        ...
        <div className={styles.select}>
          <Select
            ...
            content={status}
            handleChange={handleChange}
            options={[{ status: 'Queue' }, { status: 'Development' }, { status: 'Done' }]}
            selected={{
        ...
        <div className={styles.select}>
          <Select
            ...
            content={status}
            handleChange={handleChange}
            options={[{ status: 'Easy' }, { status: 'Normal' }, { status: 'Hard' }]}
            selected={{
              status: priority.charAt(0).toUpperCase() + priority.slice(1)
            }}
          />
        </div>
