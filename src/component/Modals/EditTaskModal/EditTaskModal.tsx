...
const EditTaskModal = ({ header, description, endDate, taskId, projectId }: EditModal) => {
    ...
    return (
        <ModalWrapper isOpen={modal} onClose={() => { dispatch(closeEditModal()) }}>
            <div>
                <Input label={'Название'} placeholder={'Задача'} maxLength={20} required projectName={header} name={'header'} handleChange={handleChange} />
                <Input textarea label={'Описание'} placeholder={'Описание'} maxLength={2000} required projectName={description} name={'description'} handleChange={handleChange} />
